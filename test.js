...
test('get heroes by role', t => {
  t.is(m.getHeroes({ roles: { carry: 1 } }).length, 62);
  t.is(m.getHeroes({ roles: { carry: 2 } }).length, 41);
...
test('get heroes by attack type', t => {
  t.is(m.getHeroes({ attack: 'melee' }).length, 54);
  t.is(m.getHeroes({ attack: 'ranged' }).length, 59);
});
...
test('get heroes by primary attribute', t => {
  ...
  t.is(m.getHeroes({ primaryAttribute: 'agility' }).length, 36);
  t.is(m.getHeroes({ primaryAttribute: 'intellect' }).length, 40);
});
